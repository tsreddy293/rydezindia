...
function buildVisibleSteps(ctx: {
  ...
  if (ctx.isCancelled) {
    const steps: TimelineStep[] = ["created", "payment", "owner_confirmed", "cancelled"];
    if (ctx.hasRefund) steps.push("refund_completed");
    return steps;
  }
...
function stepState(
  ...
  switch (step) {
    case "created":
      return "complete";
    case "payment":
      if (payment === "paid" || payment === "partial" || booking !== "pending") return "complete";
      if (booking === "pending") return "current";
      return "upcoming";
    ...
    case "refund_completed":
      ...
      if (ctx.isCancelled && ctx.hasRefund) return "current";
      return "upcoming";
    default:
      return "upcoming";
  }
...
export default function BookingTimeline({
  ...
  const visibleSteps = buildVisibleSteps({ isCancelled, hasRefund });
